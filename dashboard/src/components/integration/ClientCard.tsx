...
/** Inline copyable code row — label + monospace value + copy button */
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2 px-3 py-2 rounded-md border border-[var(--border)] bg-[var(--background)]">
        <code className="flex-1 text-xs font-mono text-[var(--foreground)] truncate select-all">
          {value}
        </code>
        <button
          onClick={async () => {
...
export function ClientCard({
  ...
  // Fix #3: re-fetch on model change — clear stale data first, no !previewData guard
  useEffect(() => {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      setStatus({ ok: false, msg: (e instanceof Error ? e.message : null) || "Failed" });
    } finally {
      setApplying(false);
  ...
  return (
    <Card className={`${!client.detected ? "opacity-70" : ""}`}>
      <CardContent className="p-4 space-y-3">
        ...
        {/* Fix #4: API key as copyable row */}
        {apiKey && <CodeRow label="API Key" value={apiKey} />}
